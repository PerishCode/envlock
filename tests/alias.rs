...
fn alias_run_executes_explicit_entrypoint() {
    ...
    std::fs::write(
        &profile,
        r#"{"injections":[{"type":"env","vars":{"ENVLOCK_PROFILE":"from-run"}}]}"#,
    )
    .expect("profile should be written");
    ...
    assert!(run.status.success());
    let stdout = String::from_utf8(run.stdout).expect("stdout should be UTF-8");
    assert!(stdout.contains("export ENVLOCK_PROFILE='from-run'"));
}
...
fn bare_alias_name_is_not_supported() {
    ...
    let profile = envlock_home.join("profiles/work.json");
    let default_profile = envlock_home.join("profiles/default.json");
    std::fs::write(
        &profile,
        r#"{"injections":[{"type":"env","vars":{"ENVLOCK_PROFILE":"from-run"}}]}"#,
    )
