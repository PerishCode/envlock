...
fn build_injections(specs: Vec<InjectionSpec>) -> Vec<RuntimeInjection> {
    ...
    for spec in specs {
        match spec {
            InjectionSpec::Env(cfg) if cfg.enabled => {
                injections.push(RuntimeInjection::Env(EnvInjection::new(cfg)));
            }
            InjectionSpec::Symlink(cfg) if cfg.enabled => {
...
mod tests {
    ...
    fn skip_disabled_env_injection() {
        let specs = vec![
            InjectionSpec::Env(crate::config::EnvConfig {
                enabled: false,
    ...
    fn fail_validation_when_env_key_is_empty() {
        let specs = vec![InjectionSpec::Env(crate::config::EnvConfig {
            enabled: true,
