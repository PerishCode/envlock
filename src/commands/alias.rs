...
pub fn run_list() -> Result<()> {
    ...
    let store = AliasStore::load(&envlock_home)?;
...
pub fn run_append(options: AliasAppendOptions) -> Result<()> {
    ...
    let envlock_home = resolve_envlock_home(&RawEnv::from_process())?;
    let mut store = AliasStore::load(&envlock_home)?;
    ...
    let path = store.save(&envlock_home)?;
    println!(
        "Appended alias: {} -> {} ({})",
        options.name,
        options.profile,
...
pub fn resolve_profile_for_alias(name: &str) -> Result<Option<String>> {
    let envlock_home = resolve_envlock_home(&RawEnv::from_process())
        .context("unable to resolve envlock home for alias lookup")?;
    let store = AliasStore::load(&envlock_home)?;
